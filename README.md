# terraform-ansible-docker_compose
...
3.  **Просмотрите `terraform/terraform.tfvars`**: При необходимости измените такие параметры, как `zone`, `vps_count`, `cores_count`, `memory_count` и т.д.
4.  **SSH-ключ**: Убедитесь, что ваш публичный SSH-ключ находится по адресу `~/.ssh/id_ed25519.pub`, или обновите метаданные `ssh-keys` в `terraform/main.tf`, чтобы указать правильный путь.
5. (Опционально) Ниже приведены настройки по-умолчанию, дли их изменения создайте файл `terraform.tfvars` в директории `terraform/` и укажите в нем необходимые параметры, например `vps_count = 4`
...
*   **Полное развертывание (Terraform + Ansible)**:
    ...
    3.  Запуск Ansible-плейбука для настройки ВМ и развертывания приложения.
...
│   ├── terraform.tfvars      # Значения переменных по умолчанию
...
*   **Повторный вывод IP адреса балансировщика и созданных машин после заврешения развертывания**:
*   **Интеграция CI/CD**: Автоматизация развертывания с использованием GitLab CI/CD
*   **Мониторинг и логирование**: Интеграция с сервисами Grafana + Promtheus + loki
